...
func (r *Resolver) BuildSystemPrompt() string {
	...
	sb.WriteString(fmt.Sprintf(systemPromptProjectLine, r.cfg.Project.Name))
	if r.cfg.Project.Language != "" {
		sb.WriteString(fmt.Sprintf(systemPromptLanguageLine, r.cfg.Project.Language))
	}
	if r.cfg.DomainContext.Domain != "" {
		sb.WriteString(fmt.Sprintf(systemPromptDomainLine, r.cfg.DomainContext.Domain))
		if r.cfg.DomainContext.Description != "" {
			sb.WriteString(fmt.Sprintf(systemPromptContextLine, r.cfg.DomainContext.Description))
		}
	}
	...
	if len(r.cfg.DomainContext.Rules) > 0 {
		sb.WriteString(systemPromptDomainHeader)
		for _, dr := range r.cfg.DomainContext.Rules {
